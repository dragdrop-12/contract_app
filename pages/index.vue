...
import Cropper from 'cropperjs'; // Ensure CropperJS is installed
import 'cropperjs/dist/cropper.css'; // Ensure CropperJS is installed
...
async function cropImage() {
  ...
  // Update the image element's src with the new data URL
  if (imageElement.value) {
    imageElement.value.src = croppedDataURL;
  }

  // Optionally update fileUri and file ref if needed elsewhere in your logic
  fileUri.value = croppedDataURL;
  file.value = new File([await fetch(croppedDataURL).then(r => r.blob())], "cropped-image.png", { type: "image/png" });
}
...
// async function updateCroppedImage(dataURL) {
//   const blob = await fetch(dataURL).then(r => r.blob());
//   file.value = new File([blob], "cropped-image.png", { type: "image/png" });
//   fileUri.value = dataURL;
//   if (imageElement.value) {
        ...
        <div class="grid place-items-center w-full h-full rounded-2xl text-zinc-800">
          ...
          >
            <div class="flex flex-col gap-2">
              ...
              <img :src="fileUri" id="image" ref="imageElement" class="object-contain max-w-100vw sm:max-w-[calc(100vw-20rem)] sm:h-[calc(100vh-12rem)]">
            </div>
            ...
            <div class="flex flex-col gap-2 pointer-events-none">
              ...
              >
                {{ isCropping ? 'Finish Cropping' : 'Edit Image' }}
              </button>
